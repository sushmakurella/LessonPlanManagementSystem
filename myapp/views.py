...
def mainpage(request,name):
     ...
     usname='sushma'
     return render(request,'mainpage.html',{'usname':usname})
def inputform(request):
    return render(request,'inputform.html')
def findpo(i,j,coval,poval):
    if(coval[i]>poval[j]):
        return 3
    elif(coval[i]<poval[j]):
        return 1
    return 2
        ...
        if lessonplanBatch.objects.filter(course_code=coursecode).exists():
             ...
             for i in range(1,numco+1):
                  ...
                  if(request.POST["kl"+str(i)]):
                       ...
                       #console
                       cokls.append(int(knowledge_level[2]))
            ...
            #***********lecture plan **************
             for i in range(numco):
                  ...
                  for j in range(1,temp+1):
                       ...
                       co1_lp.save();
             #******* co&pso matrix***************
             poval=[3,4,5,5,3,3,3,3,6,2,6,1,3,3]
             psomatrix=[[0 for i in range(14)] for j in range(numco)]
             ...
             for i in range(14):
                 ...
                 for j in range(numco):
                     if(psomatrix[j][i]!=0):
                         ...
                         sum1+=psomatrix[i][j]
                 if(count>0):
                     coursepso[i]=math.ceil(sum1/count)
             co_pso=co_pso_Matrix.objects.create(cos="Course",po1=coursepso[0],po2=coursepso[1],po3=coursepso[2],po4=coursepso[3],po5=coursepso[4],po6=coursepso[5],po7=coursepso[6],po8=coursepso[7],po9=coursepso[8],po10=coursepso[9],po11=coursepso[10],po12=coursepso[11],pso1=coursepso[12],pso2=coursepso[13],course_code=coursecode)
             co_pso.save();
...
def viewplan(request):
        ...
        return render(request,'viewplan.html',d)
def inputcoursecode(request):
     return render(request,'inputcoursecode.html')
def updatecoursecode(request):
     return render(request,'updatecoursecode.html')
...
def updateinput(request):
    ...
    #course_code=models.CharField(max_length=15)            
            ...
            #***********lecture plan **************
             ...
             lp_ins=details_of_instructors.objects.create(sno='1',name=name,designation=designation,year=year,section=section,contactno=contactno,email=email,course_code=coursecode)
             lp_ins.save();
        return redirect('updatecoursecode')           
    return render(request,"storeinput.html")
...
def logint(request):
    if request.method == 'POST':
        ...
        if user is not None:
            # User is authenticated, log them in or do further processing
            # ...
            return render(request,"logintvalidate.html")
            
        else:
            # Invalid credentials, handle the error
            return render(request,"logintfail.html")
            # ...
...
from .utils import extract_text_from_pdf, extract_course_outcomes, extract_units

def upload_pdf(request):
    """Handles PDF upload, extraction, and displays the results."""
    if request.method == "POST":
