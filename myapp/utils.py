...
def extract_units(text):
    """Extracts units and their syllabus, stopping before 'Text Books'."""
    unit_pattern = re.findall(r"(UNIT-\w+):\s*(.*?)(?=UNIT-\w+:|Text\s*Books:|\Z)", text, re.DOTALL)
    unit_dict = {}
    ...
    for unit in unit_pattern:
        ...
        syllabus = []
        for line in lines[1:]:
            if "Text Books" in line:  # Stop extraction if 'Text Books' appears
                break
